...
create_filter(char instr, int icnt)
{
    ...
    /* NOTE: Nowadays, the kernel appears to be doing some optimization of the
       ...
       on the execution time. Presumably, the optimization is eliding away the
       no-op instructions. This was not always the case: in older kernels,
       varying the size of such filters did change the execution time. I have
       not determined when the change occurred (or the location of the relevant
       code in the kernel), but some quick testing suggests that the change was
       no earlier than kernel 5.11 and no later than kernel 5.15. */
